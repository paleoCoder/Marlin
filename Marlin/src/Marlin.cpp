...
// BIGTREE miniUPS detect
#if ENABLED(BIGTREE_MINI_UPS)
void mini_ups_detect(void) {
  if(READ(POWER_LOSS_PIN) == POWER_LOSS_STATE)
  {
    ...
    SERIAL_ERROR_MSG("Power failed!");

    float default_max_speed[] = DEFAULT_MAX_FEEDRATE;
    int graps = 1000000.f / default_max_speed[E_AXIS] / planner.settings.axis_steps_per_mm[E_AXIS] / 2;
    WRITE(E0_ENABLE_PIN, E_ENABLE_ON);
    WRITE(E0_DIR_PIN, INVERT_E0_DIR);
    for (int i = 0; i < 10*planner.settings.axis_steps_per_mm[E_AXIS]; i++) {
      WRITE(E0_STEP_PIN, LOW);
      DELAY_US(graps);
      WRITE(E0_STEP_PIN, HIGH);
      DELAY_US(graps);
    }
    graps = 1000000.f / default_max_speed[Z_AXIS] / planner.settings.axis_steps_per_mm[Z_AXIS] / 2;
    WRITE(Z_ENABLE_PIN, Z_ENABLE_ON);
    WRITE(Z_DIR_PIN, !INVERT_Z_DIR);
    for (int i = 0; i < 10 * planner.settings.axis_steps_per_mm[Z_AXIS]; i++)
    {
